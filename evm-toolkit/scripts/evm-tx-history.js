...
async function queryTransactionHistory(address, apiKey, limit = 10) {
  ...
  try {
    ...
    for (let i = 0; i < transactions.length; i++) {
      ...
      const direction = isContractCreation ? '📝 创建合约' : (isIncoming ? '📥 接收' : '📤 发送');
      const counterparty = isContractCreation ? '(合约创建)' : (isIncoming ? tx.from : to);
...
⚠️  注意：
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--address' && args[i + 1]) {
      ...
      if (isNaN(limit) || limit < 1) { console.error('❌ --limit 必须是正整数'); process.exit(1); }
      i++;
    }
  }
  ...
  if (!apiKey) {
    console.error('❌ 请指定 --api-key（从 https://etherscan.io/apis 获取）');
    process.exit(1);
  }
